...
def region_center_distance_sq(region, x, y):
...
def make_context_override_kwargs(
    context,
    area=None,
    region=None,
    space=None,
...
def resolve_region_data_for_region(
    ...
    try:
        ...
        if is_region_view3d(region_data):
            return region_data
    except Exception:
        pass
    ...
    if hasattr(context, "temp_override"):
        kwargs = make_context_override_kwargs(
            context,
            area=area,
            region=region,
            space=space,
...
def find_view3d_area_region_under_mouse(
    ...
    if (
        ...
        region = min(
            window_regions,
            key=lambda item: region_center_distance_sq(
                item,
                mouse_x,
                mouse_y,
            ),
